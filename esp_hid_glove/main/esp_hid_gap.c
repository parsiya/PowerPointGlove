...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            ...
            // Parsia: Change passcode
            pkey.passkey = 654321; // This is the passkey to be entered on peer
            ...
            ESP_LOGI(TAG, "Input not supported passing -> 654321"); // Parsia: Change passcode
            pkey.action = event->passkey.params.action;
            pkey.passkey = 654321; // Parsia: Change passcode
            rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
            ESP_LOGI(TAG, "ble_sm_inject_io result: %d", rc);
        }
        return 0;
