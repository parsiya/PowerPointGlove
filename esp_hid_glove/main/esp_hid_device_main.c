...
// Parsia: Keycodes
// Ref: https://gist.github.com/MightyPork/6da26e382a7ad91b5496ee55fdc73db2
...
// Parsia: ZZ candidate for removal.
...
// // Parsia: This will send specific keys based on the input. Because we only care
// // about a few keys here.
// void send_keyboard_glove(uint8_t key)
// {
//     uint8_t buffer[8] = {0};
//     buffer[2] = key;
//     static uint8_t key_release[8] = {0};
//     esp_hidd_dev_input_set(s_ble_hid_param.hid_dev, 0, 1, buffer, 8);
//     /* send the keyrelease event with sufficient delay */
//     vTaskDelay(50 / portTICK_PERIOD_MS);
//     esp_hidd_dev_input_set(s_ble_hid_param.hid_dev, 0, 1, key_release, 8);
// }

void power_glove_to_keyboard(void *pvParameters)
{
    // Parsia: ZZ my keyboard task
    const char *TAG = "power_glove_to_keyboard";
    ESP_LOGI(TAG, "starting power_glove_to_keyboard task");
    ...
    // Select: KEY_MEDIA_CALC
    // A: KEY_MEDIA_COFFEE ?? What does it even do?
    // B: KEY_MEDIA_WWW
    // Right: KEY_RIGHT
    // Left: KEY_LEFT
    // Up: KEY_UP
    ...
    ButtonMapping mappings[] = {
        ...
        {BUTTON_SELECT, KEY_MEDIA_CALC},
        {BUTTON_A, KEY_MEDIA_COFFEE},
        {BUTTON_B, KEY_MEDIA_WWW},
        {BUTTON_UP, KEY_UP},
    ...
    for (;;) {
        ...
        for (int i = 0; i < sizeof(mappings) / sizeof(mappings[0]); i++) {
            if (!(incomingData & mappings[i].buttonMask)) {
                ESP_LOGI(TAG, "sending %" PRIu8 "to keyboard", mappings[i].keyboardEvent);
                send_keyboard_glove(mappings[i].keyboardEvent);
...
void ble_hid_task_start_up(void)
{
    ...
    // ZZ: Set battery level to 100% so we do not get a warning from Windows.
    // TODO: Supposedly we can read PIN 35 for the connected battery. ADC?
...
// Parsia: ZZ keyboard task.
#elif CONFIG_EXAMPLE_HID_DEVICE_ROLE == 2
    /* Nimble Specific */
    xTaskCreate(power_glove_to_keyboard, "power_glove_to_keyboard", 3 * 1024, NULL, configMAX_PRIORITIES - 3,
                &s_ble_hid_param.task_hdl);
// Parsia: ZZ mouse task.
#elif CONFIG_EXAMPLE_HID_DEVICE_ROLE == 3
    /* Nimble Specific */
    xTaskCreate(power_glove_to_mouse, "power_glove_to_mouse", 3 * 1024, NULL, configMAX_PRIORITIES - 3,
                &s_ble_hid_param.task_hdl);
#endif
}
...
void app_main(void)
{
    ...
    // Parsia: ZZ startup code is here.
    powerglove_startup();
